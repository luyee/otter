...
public class ZkClientx implements Watcher {
	...
	private static LoadingCache<String, ZkClientx> clients = CacheBuilder.newBuilder().maximumSize(1000)
			.build(new CacheLoader<String, ZkClientx>() {

				@Override
				public ZkClientx load(String servers) throws Exception {
	...
	/**
	 * Updates data of an existing znode. The current content of the znode is
	 * passed to the {@link DataUpdater} that is passed into this method, which
	 * returns the new content. The new content is only written back to
	 * ZooKeeper if nobody has modified the given znode in between. If a
	 * concurrent change has been detected the new data of the znode is passed
	 * to the updater once again until the new contents can be successfully
		...
		return retryUntilConnected(new Callable<List<String>>() {
			...
			public List<String> call() throws Exception {
				...
				try {
					...
					// ignore, the "exists" watch will listen for the parent
					// node to appear
				}
				return null;
			}
