...
public class StatsRemoteServiceImpl implements StatsRemoteService {
    ...
    private LoadingCache<Long, AvgStat>                             delayStats;//延迟时间
    private LoadingCache<Long, Map<ThroughputType, ThroughputStat>> throughputStats;//吞吐量
    ...
    public StatsRemoteServiceImpl(){
        ...
        delayStats =CacheBuilder.newBuilder()
                .maximumSize(1000)    // 最多可以缓存1000个key
                .build(
                    new CacheLoader<Long, AvgStat>() {
                        public AvgStat load(Long pipelineId) {
                        	return new AvgStat();
                        }
                    });
//        new MapMaker().makeComputingMap(new Function<Long, AvgStat>() {
//
//            public AvgStat apply(Long pipelineId) {
//                return new AvgStat();
//            }
//        });
        throughputStats =CacheBuilder.newBuilder()
                .maximumSize(1000)    // 最多可以缓存1000个key
                .build(
                    new CacheLoader<Long, Map<ThroughputType, ThroughputStat>>() {
                        public Map<ThroughputType, ThroughputStat> load(Long pipelineId) {
                        	return new HashMap<ThroughputType, ThroughputStat>();
                        }
                    });
        		
//        		new MapMaker().makeComputingMap(new Function<Long, Map<ThroughputType, ThroughputStat>>() {
//
...
//        });
    ...
    private void flushDelayStat() {
        synchronized (delayStats) {
            // 需要做同步，避免delay数据丢失
            for (Map.Entry<Long, AvgStat> stat : delayStats.asMap().entrySet()) {
                if (stat.getValue().count.get() > 0) {
                    ...
                    delay.setMaxDelayTime(stat.getValue().getMax());
                    delay.setMinDelayTime(stat.getValue().getMin());
                    delay.setDelayNumber(0L);
                    delayStatService.createDelayStat(delay);
                }
            }
            delayStats.invalidateAll();//cleanUp();//clear();
        }
    }

    private void flushThroughputStat() {
        synchronized (throughputStats) {
            Collection<Map<ThroughputType, ThroughputStat>> stats = throughputStats.asMap().values();
    ...
    public static class AvgStat {
        ...
        public void merge(DelayStat stat) {
            ...
            if (stat.getDelayTime()>this.max.get()){
            	this.max.set(stat.getDelayTime());
            }else{
            	this.min.set(stat.getDelayTime());
            }
            logger.info("总计时间： number:"+number+"ms  deplaytime:"+stat.getDelayTime()+"ms");
        }
