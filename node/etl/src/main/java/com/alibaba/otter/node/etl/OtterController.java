...
/**
 * 管理和维护对应node机器内的S.E.T.L任务，实时接收manager推送的NodeTask调度信息，可查看
 * {@linkplain NodeTaskService}
 * 
	...
	private void checkNidVaild(String nid) {
		...
		if (!AddressUtils.isHostIp(nodeIp)) {
			throw new IllegalArgumentException(String.format(
					"node[%s] ip[%s] port[%s] , but your host ip[%s] is not matched!", nid, nodeIp, nodePort, hostIp));
		}
	}
