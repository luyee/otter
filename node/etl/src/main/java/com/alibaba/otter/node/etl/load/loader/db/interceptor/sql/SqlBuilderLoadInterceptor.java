...
import com.alibaba.otter.node.etl.common.db.dialect.NosqlTemplateUtil;
...
//            }
    ...
    public boolean beforeForKafka(DbLoadContext context, EventData currentData) {
    	DataMediaSource dataMediaSource = context.getDataMediaSource();
//    	if(dataMediaSource.getType().isKafka()){
//    		return false;
//    	}
...
//       SqlTemplate sqlTemplate = dbDialect.getSqlTemplate();
        ...
        // 注意insert/update语句对应的字段数序都是将主键排在后面
        if (type.isInsert()) {
            if (CollectionUtils.isEmpty(currentData.getColumns())) { // 如果表为全主键，直接进行insert
                                                                                                                 // sql
                sql = NosqlTemplateUtil.getInsertSql(schemaName,
                    currentData.getTableName(),
                    buildColumnNames(currentData.getKeys()),
                    buildColumnNames(currentData.getColumns()));
            } else {
                sql = NosqlTemplateUtil.getMergeSql(schemaName,
                    currentData.getTableName(),
                    buildColumnNames(currentData.getKeys()),
                    buildColumnNames(currentData.getColumns()),
                    new String[] {},
            ...
            if (rowMode && !existOldKeys) {// 如果是行记录,并且不存在主键变更，考虑merge sql
                ...
                sql = NosqlTemplateUtil.getUpdateSql(schemaName, currentData.getTableName(), keyColumns, otherColumns);
            }
        } else if (type.isDelete()) {
            sql = NosqlTemplateUtil.getDeleteSql(schemaName,
                currentData.getTableName(),
                buildColumnNames(currentData.getKeys()));
        }
