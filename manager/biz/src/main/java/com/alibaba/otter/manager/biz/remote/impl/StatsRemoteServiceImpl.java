...
public class StatsRemoteServiceImpl implements StatsRemoteService {
	...
	public StatsRemoteServiceImpl() {
		...
		delayStats = CacheBuilder.newBuilder().maximumSize(1000).build(new CacheLoader<Long, AvgStat>() {

			public AvgStat load(Long pipelineId) {
				return new AvgStat();
			}
		});
		throughputStats = CacheBuilder.newBuilder().maximumSize(1000)
				.build(new CacheLoader<Long, Map<ThroughputType, ThroughputStat>>() {
	...
	private void flushDelayStat() {
		synchronized (delayStats) {
			...
			delayStats.invalidateAll();//clear();
		}
	}
	...
	private void flushThroughputStat() {
		synchronized (throughputStats) {
			...
			throughputStats.invalidateAll();//.clear();
		}
	}
