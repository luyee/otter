...
public class DefaultAlarmController implements AlarmController {
    ...
    private LoadingCache<PoolKey, PoolValue> pool       =CacheBuilder.newBuilder()
            .maximumSize(1000)    // 最多可以缓存1000个key
            .expireAfterWrite(1, TimeUnit.HOURS).build(new CacheLoader<PoolKey, PoolValue>(){ 
				@Override
				public PoolValue load(PoolKey key) throws Exception {
					return new PoolValue(System.currentTimeMillis());
				}
			});  //new MapMaker().expireAfterWrite(1, TimeUnit.HOURS).makeMap();
